...
/** Watches the snapshot for big moments (takeovers, a season ending) and
 * turns them into transient toasts. Skips whatever state already existed
 * on the first snapshot received, so joining mid-session doesn't dredge up
 * stale events. */
export function useToasts(snapshot: GameStateSnapshot | null) {
